...
class NuageEventMonitor
  def self.new(options = {})
    # plugin initializer
    self == NuageEventMonitor ? event_monitor(options) : super
  end
  ...
  # See NuageEventMonitor.new for details on event monitor selection
  def initialize(_options = {})
    # See NuageEventMonitor.new
    raise NotImplementedError, "Cannot instantiate NuageEventMonitor directly."
  end
  ...
  def self.event_monitor_selected_class(options)
    ...
    when :ceilometer
      NuageVSDEventMonitor
    else
      NuageNullEventMonitor
    end
  end
  ...
  def self.available_event_monitor(event_monitor_class, options)
    ...
    event_monitor_class = NuageNullEventMonitor unless monitor_available
    event_monitor_class
  end

  # Select the best-fit plugin, or OpenstackNullEventMonitor if no plugin will
  # work Return the plugin instance
  # Caches plugin instances by nuage provider
  ...
  def nuage_event(_delivery_info, metadata, payload)
    NuageEvent.new(payload,
